...
class Dompdf
{
    ...
    public function __construct($options = null)
    {
        ...
        if (
            ...
            ($version = file_get_contents($versionFile)) !== false &&
            $version !== '$Format:<%h>$'
        ) {
        ...
        $this->setFontMetrics(
            new FontMetrics($this->getCanvas(), $this->getOptions())
        );
    ...
    public function loadHtmlFile($file)
    {
        ...
        if (
            !$this->options->isRemoteEnabled() &&
            ($protocol != '' && $protocol !== 'file://')
        ) {
            throw new Exception(
                'Remote file requested, but remote file download is disabled.'
            );
        }
        ...
        if ($protocol == '' || $protocol === 'file://') {
            ...
            if ($chroot && strpos($realfile, $chroot) !== 0) {
                throw new Exception(
                    "Permission denied on $file. The file could not be found under the directory specified by Options::chroot."
                );
            }
        ...
        list($contents, $http_response_header) = Helpers::getFileContent(
            $file,
            $this->httpContext
        );
        $encoding = 'UTF-8';
        ...
        if (isset($http_response_header)) {
            foreach ($http_response_header as $_header) {
                if (
                    preg_match(
                        '@Content-Type:\s*[\w/]+;\s*?charset=([^\s]+)@i',
                        $_header,
                        $matches
                    )
                ) {
    ...
    public function loadHtml($str, $encoding = 'UTF-8')
    {
        ...
        foreach ($metatags as $metatag) {
            if (preg_match($metatag, $str, $matches)) {
                if (
                    isset($matches[1]) &&
                    in_array($matches[1], $known_encodings)
                ) {
        ...
        if (
            isset($document_encoding) &&
            in_array(strtoupper($document_encoding), ['UTF-8', 'UTF8']) ===
                false
        ) {
            $str = preg_replace('/charset=([^\s"]+)/i', 'charset=UTF-8', $str);
        } elseif (
            isset($document_encoding) === false &&
            strpos($str, '<head>') !== false
        ) {
            $str = str_replace(
                '<head>',
        ...
        if (
            $this->options->isHtml5ParserEnabled() &&
            class_exists('HTML5_Tokenizer')
        ) {
            ...
            $quirksmode =
                $tokenizer->getTree()->getQuirksMode() >
                HTML5_TreeBuilder::NO_QUIRKS;
        } else {
    ...
    private function processHtml()
    {
        ...
        if (
            $base_nodes->length &&
            ($href = $base_nodes->item(0)->getAttribute('href'))
        ) {
            list(
                $this->protocol,
                $this->baseHost,
                $this->basePath,
            ) = Helpers::explode_url($href);
        }
    ...
    public function render()
    {
        ...
        if ($logOutputFile) {
            if (
                !file_exists($logOutputFile) &&
                is_writable(dirname($logOutputFile))
            ) {
                touch($logOutputFile);
            }
        ...
        $defaultOptionPaperSize = $this->getPaperSize(
            $options->getDefaultPaperSize()
        );
        ...
        if ($options->isFontSubsettingEnabled() && $canvas instanceof CPDF) {
            foreach ($this->tree->get_frames() as $frame) {
                ...
                if ($node->nodeName === '#text') {
                    $chars =
                        mb_strtoupper($node->nodeValue) .
                        mb_strtolower($node->nodeValue);
                    $canvas->register_string_subset(
                        $style->font_family,
                        $chars
                    );
                    continue;
                ...
                if ($style->display === 'list-item') {
                    $chars = ListBullet::get_counter_chars(
                ...
                if ($frame->get_node()->nodeName == 'dompdf_generated') {
                    ...
                    $chars =
                        mb_strtoupper($style->content) .
                        mb_strtolower($style->content) .
                        mb_strtoupper($decoded_string) .
                        mb_strtolower($decoded_string);
                    $canvas->register_string_subset(
                        $style->font_family,
                        $chars
                    );
    ...
    private function write_log()
    {
        ...
        $out = sprintf(
            "<span style='color: #000' title='Frames'>%6d</span>" .
                ...
                ($this->quirksmode
                    ? "<span style='color: #d00'> ON</span>"
                    : "<span style='color: #0d0'>OFF</span>") .
                '</span><br />',
            $frames,
            $memory,
            $time
